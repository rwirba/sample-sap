...
echo "🌐 Minikube IP: ${MINIKUBE_IP}"
echo "🔢 NodePort: ${NODE_PORT} (TargetPort: ${TARGET_PORT})"

if curl -s --max-time 5 "http://${MINIKUBE_IP}:${NODE_PORT}" >/dev/null; then
  echo "✅ Local connection test passed."
else
  echo "⚠️ Warning: Service not responding locally — tunnel may return 502."
fi
...
# --- Ensure DNS record is routed correctly
cloudflared tunnel route dns "${APP_NAME}-tunnel" "${HOSTNAME}" || true
